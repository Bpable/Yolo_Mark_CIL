...
std::atomic<bool> increase_size;
std::atomic<bool> decrease_size;

std::atomic<bool> delete_file(false);
	...
	try
	{
		...
		do {
			...
			if (old_trackbar_value != trackbar_value || exit_flag)
			{
               ...
               //
               // Did have it as SCALAR::ALL(255) originally, also added Line thickness 1 & LineTypes::FILLED, Changed Hershey_COMPLEX_SMALL ->HERHSEY_PLAIN
               // changed from pont size from 0.5 to 0.7. Try to Keep names within 13 characters
               //
               putText(dst_roi, jpg_filenames[trackbar_value + i], Point2i(0, 10), FONT_HERSHEY_PLAIN, 0.7, Scalar(0, 255, 0), 1, LineTypes::FILLED);
         ...
         {
            std::string const count_str = "Label Count: " + std::to_string(current_coord_vec.size());

            putText(full_image_roi, count_str, Point2i(800, 21), FONT_HERSHEY_DUPLEX, 0.8, Scalar(10, 50, 10), 3);
            putText(full_image_roi, count_str, Point2i(800, 21), FONT_HERSHEY_DUPLEX, 0.8, Scalar(20, 120, 60), 2);
            putText(full_image_roi, count_str, Point2i(800, 21), FONT_HERSHEY_DUPLEX, 0.8, Scalar(50, 200, 100), 1);
            ...
            //
            // When I figure out the increase & decrease size. Then uncomment this
            //
            putText(full_image_roi, "d - delete_file", Point2i(0, 115), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(50, 10, 10), 2);
			}
			else
			{
				putText(full_image_roi,
         ...
         //
         // Putting these in here as cases for now
